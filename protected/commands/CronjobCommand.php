              ...
              if (isset($languages->infoTributaria->claveAcceso)){
                 ...
                 foreach ($languages->children() as $key=>$info)
                {
                     if ($key=='infoFactura' or $key=='infoNotaDebito' or $key=='infoNotaCredito'){ 
                         ...
                         if ($info->tipoIdentificacionComprador==5) $datos->tipoIdentificacionComprador='C';
                         else  if ($info->tipoIdentificacionComprador==4) $datos->tipoIdentificacionComprador='R';
                         else $datos->tipoIdentificacionComprador='P';
    ...
    function GuardarArchivo($data,$path){
        ...
        $usuario= Usuarios::model()->find("usr_ruc='{$data->ruc}'");
                 if ($usuario==null)
                {
                   ...
                   if(!empty ($data->telefono))$User->usr_telefono=(string)$data->telefono;
                   if(!empty ($data->celular))$User->usr_telefono1=(string)$data->celular;
                   if(! $User->save()) {  
                ...
                else 
                {
                    ...
                    if(!empty ($data->telefono))$usuario->usr_telefono=(string)$data->telefono;
                    if(!empty ($data->celular))$usuario->usr_telefono1=(string)$data->celular;
                ...
                if ($usr_codigo!=null)
                        ...
                        if (count($modelDoc)>0){
                         ...
                         if(!$modelDoc->save()){  $respuesta=  false;}
                            ...
                            if (isset ($data->subtotal)) $model->doc_subtotal=$data->subtotal;
                            if (isset ($data->iva)) $model->doc_iva=$data->iva;
                            if (isset ($data->total)) $model->doc_total=$data->total;
                            if (isset ($data->matrizador)) $model->doc_matrizador=$data->matrizador;
                            //Fin
